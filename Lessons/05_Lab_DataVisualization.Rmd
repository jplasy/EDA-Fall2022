...
Temps.by.day <- ggplot(subset(PeterPaul.chem.nutrients, depth ==0), aes(x = sampledate, y = temperature_C)) + 
  geom_point(aes(color=year4)) +  facet_wrap(vars(lakename), nrow = 2) +ylab(expression(" Surface Temp (C)"))
print(Temps.by.day)

#function subset is to get surface temp only (depth is zero) or you can filter the data beforehand 
...
ggplot(PeterPaul.chem.nutrients, aes(x = sampledate, y = temperature_C)) + 
  geom_point(aes(color=depth, size =0.5))  + scale_color_viridis(option = "magma", direction = -1) +
scale_x_date(date_breaks= "5 years", date_labels= "%Y") + ylab("Temperature C") 
