...
knitr::opts_chunk$set(tidy.opts=list(width.cutoff=80), tidy=TRUE) #this will set up a width for the code in the pdf, if a comment goes behind then u have to manually fix it
```
...
NTL.phys.data.Tues.years <- filter(NTL.phys.data, lakename == "Tuesday Lake" & year4 >=1990 & year4 <=1999)

NTL.phys.data.Tues.years.2 <- filter(NTL.phys.data, lakename == "Tuesday Lake" & year4 %in% c(1990:1999))

#c(1990:1999) creates a vector with all of the numbers
```
...
NTL.phys.data.processed.2 <- 
  NTL.phys.data %>%
  filter(lakename == "Tuesday Lake" & year4 %in% c(1990:1999) & month(sampledate)==6)

#the function month() is only for date objects
...
new.data <- NTL.phys.data.processed.2  %>% filter(depth==0) %>% drop_na(temperature_C) %>% summarize(mean_temp <-mean(temperature_C))
...
irradiance.data <- NTL.phys.data %>% pivot_longer(irradianceWater:irradianceDeck, names_to= "Irradiance", values_to = "radiation")


#Exercise 6: spread temperatureC into more than one column based on the depth

NTL.phys.data.ex6 <- NTL.phys.data %>% pivot_wider(names_from = "depth", values_from = "temperature_C")
