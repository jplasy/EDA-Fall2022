...
##so either create a subset so corrplot knows what to do with NAs and then use this as the dataframe, or include it below within the code
...
EPAaircorplot <- 
  ...
  select(meanLat : Ozone) %>%
  na.omit()
EPACorr <- cor(EPAaircorplot)
corrplot(EPACorr, method = "ellipse")
corrplot.mixed(EPACorr, upper = "ellipse")

#the stronger the line (shape of the circle), value, or color, the more of a correlation
#meanLat, Month, and Ozone are showing the strongest correlations, so use them for the next steps 
...
ALL <- lm(data = EPAair, PM2.5 ~ meanLat + Year + meanLng + Ozone)
summary(ALL)

#small p values means significant correlation, so Ozone, Month, MeanLng, and meanLat are significant 
#this shows very similar results to 

step(ALL)
#step tells you if you remove X, you can improve AIC... if you remove Year, you will improve the AIC, there's a one number small AIC



step(AIC)
TPmodel <- lm(data = EPAair, PM2.5 ~ meanLat + meanLng + Ozone)
summary(TPmodel)
...
#usually do post-hoc tests so you can group levels better ? 
...
#all the groups with the same letters means the mean values are equal 
...
#we can improve the plot, if we can group all the a's, b's, c's etc together!
