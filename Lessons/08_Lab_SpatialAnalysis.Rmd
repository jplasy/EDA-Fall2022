...
upper_sf<- st_read('./Data/Spatial/NCHUC8.shp') %>% 
...
mapview(huc8_sf, col.region ='orange', alpha.regions=1) + mapview(upper_sf, col.region= 'purple', alpha.regions=1)



#ggplot() +geom_sf(data=huc8_sf, fill = Orange)+geom_sf(data=upper_sf, fill = Purple)
...
wind_raw <- read.csv('https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.cs', colClasses = c('HUC_12' = 'factor')) %>%
...
ggplot(data=huc_8_sf_join) + geom_sf(aes(fill=SumWindEnergy)) +
  labs(title= "wind energy by HUS 8", fill= "MW/year")
